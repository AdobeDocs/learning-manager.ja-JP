  ...
  <tr>
   ...
   <td>
    ...
    <ul>
     ...
     <li>SCORM 2004</li>
     <li>AICC</li>
     <li>Prime でサポートされている xAPI</li>
    </ul>
    <p><b>重要：学習者アシスタントおよびSCORMベースの評価</b></p>
    <p>アカウントで学習者アシスタントが有効になっており、学習者アシスタントのコンテンツソースとして設定されたカタログにSCORM / xAPIパッケージをアップロードする場合は、次の点に注意してください。</p>
    <ul>
      <li>学習者は、学習者アシスタントを使用して、これらのパッケージ内のコンテンツに関する質問を行うことができます。</li>
      <li>アシスタントは、SCORMコースに組み込まれたクイズの質問に対する回答を効果的に明らかにする説明または概要を生成することができます（例えば、フィードバックを再度言い換えたり、「正しい」オプションを説明します）。</li>
   </ul>
...
## コースコンテンツのプレビュー {#previewcontentlibrary}

コースコンテンツは、コンテンツライブラリに追加されると、プレビューすることができます。 プレビューするには、各コースの行の最後にある目のアイコンを選択します。
...
1. Adobe Learning Managerは、Adobe Learning Managerでアップロードされたビデオコンテンツのキャプションをサポートします。 作成者は、ビデオファイルと一緒に、キャプションを含むファイルをアップロードできます。
   ...
   その後、字幕が Fluidic プレーヤーに表示されます。 また、キャプションは [WCAG2.0 標準](https://www.w3.org/TR/WCAG20/)に準拠しています。
...
Adobe Learning Managerで多言語ビデオテキストトラック(VTT)がサポートされているため、作成者はビデオやオーディオコンテンツの字幕とキャプションを多言語で提供できます。 この機能により、ローカリゼーションが合理化され、世界中のオーディエンスがトレーニングにアクセスできるようになり、アクセシビリティ基準に準拠するようになります。 作成者は、プラットフォーム内で直接VTTファイルを自動生成、移動、レビュー、および編集できます。
...
1. コンテンツライブラリに移動し、[[!UICONTROL コンテンツの追加]]を選択します。
...
6. 「[!UICONTROL 保存]」を選択します。 VTTファイルの生成と移動が開始されます。
...
2. 各言語について、VTTファイルの横にあるレビューリンクを選択します。
...
* **サポートされている言語：** Adobe Learning Managerでは、英語以外の39の言語がすべてサポートされています。
...
既存のコンテンツ（つまり、コンテンツフォルダーを構成する前にアップロードされたコンテンツ）を含むコンテンツライブラリは、**公開フォルダー**&#x200B;として定義されます。 このフォルダーは廃止または削除できません。 公開フォルダーの一部であるコンテンツには、すべてのタイプの作成者がアクセスできます。 コンテンツフォルダーが設定されたら、標準作成者とカスタム作成者は、新しいコンテンツをアップロードするときに、コンテンツを配置するフォルダーを選択する必要があります。
...
新規コンテンツをアップロードする際には、アップロードプロセス中にコンテンツを保存するフォルダーを選択します。

1. 作成者としてログインし、**管理**/**コンテンツライブラリ**&#x200B;に移動するか、ホームページから&#x200B;**コンテンツを作成**&#x200B;を選択します。

2. ページの右上隅にある&#x200B;**追加** / **コンテンツ**&#x200B;を選択します。

3. コンテンツの名前と説明を入力します。

4. **[コンテンツファイルの追加]**で、コンテンツファイルをドラッグアンドドロップするか、参照してアップロードするフィールドを選択します。
   ![](assets/cl2.png)

5. [**フォルダーに追加**]で、フォルダーを参照するか、検索フィールドでフォルダーを検索します。 複数のフォルダーを選択することもできます。
   ![](assets/cl3.png)

6. 「**追加**」を選択します。

7. 必要に応じて、残りのフィールド（**期間**、**タグ**、**コンテンツの一意のID**、**有効期限**）を入力します。

8. 「**保存**」を選択します。 コンテンツファイルがコンテンツライブラリに追加され、リストの一番上に表示されます。
   ![](assets/cl4.png)

## コンテンツライブラリのフォルダパスを表示する

**コンテンツライブラリ**&#x200B;では、各コンテンツファイルのフォルダーのパス全体が示されています。たとえば、**セールストレーニング** / **自習型モジュール** / **PDFアセット**&#x200B;などです。 このパスは、階層内でのファイルの保存場所を正確に示します。

* 1つのファイルが複数のフォルダーに存在する場合、すべてのパスがコンマで区切って表示されます。

* パスが長い場合は、先頭から省略記号(...)で切り捨てられ、最も深いフォルダー名が常に表示されます。

* 公開済みコンテンツ、廃止済みコンテンツ、および下書きコンテンツはすべてコンテンツライブラリに表示され、フォルダーパスが示されます。

![](assets/cl1.png)

## フォルダーでコンテンツをフィルタリング

コンテンツライブラリの&#x200B;**フォルダー**&#x200B;フィルターを使用すると、リストを絞り込んで、特定のフォルダーに保存されているコンテンツのみを表示できます。

1. **コンテンツライブラリ**&#x200B;で、列ヘッダーの&#x200B;**フォルダー**ドロップダウンを選択します。
   ![](assets/cl5.png)
   ドロップダウンには、パブリックフォルダーとプライベートフォルダーのリストが表示されます。 パブリックフォルダーとプライベートフォルダーを同時に選択することはできません。 一方を選択すると、もう一方の選択は解除されます。

2. 特定のプライベートフォルダでフィルタするには、次の手順を実行します。
   * **レベル1フォルダー**&#x200B;を選択すると、その下にあるすべてのレベル2およびレベル3のサブフォルダーが自動的に選択されます。 これらのサブフォルダーは選択状態で表示されますが、個別に選択解除することはできません。 フィルターは、レベル1フォルダーおよびそのサブフォルダー内のすべてのコンテンツを返します。
   * **レベル2フォルダー**を選択すると、その下にあるすべてのレベル3サブフォルダーが自動的に選択されます。
     ![](assets/cl6.png)

3. ドロップダウン検索ボックスを使用して、名前でフォルダーを検索します。

4. 一度に複数のフォルダーを選択できます。

## コンテンツをフォルダーに移動 {#movecontenttofolder}

コンテンツを移動すると、現在のフォルダーからコンテンツが削除され、移動先のフォルダーに配置されます。 ダイアログで複数のプライベートフォルダーを選択して、コンテンツを移動できます。 ただし、プライベートフォルダーとパブリックフォルダーを同時に選択することはできません。 複数のプライベートフォルダーを選択することも、1つのパブリックフォルダーを選択することもできます。

1. **コンテンツライブラリ**&#x200B;で、移動する各コンテンツファイルの横にあるチェックボックスをオンにします。
2. **アクション**/**コンテンツの整理**/**コンテンツをフォルダーに移動**&#x200B;を選択します。 **コンテンツをフォルダーに移動**&#x200B;パネルが開きます。
3. コンテンツの移動先のフォルダーを選択します。
4. **コンテンツの移動**&#x200B;を選択します。 コンテンツライブラリが更新され、新しいフォルダパスが表示されます。

>[!IMPORTANT]
>
>パブリックフォルダーからプライベートフォルダーへ、またはその逆にコンテンツを移動できます。 ただし、1回の操作でパブリックフォルダーとプライベートフォルダーの両方にコンテンツを移動することはできません。 パブリックフォルダを選択すると、プライベートフォルダは選択できません。その逆も同様です。
...
コピーすると、追加のフォルダー内のコンテンツへのリンクが作成されますが、コンテンツが現在の場所から削除されることはありません。 コンテンツファイル自体は複製されません。

1. **コンテンツライブラリ**&#x200B;で、コピーする各コンテンツファイルの横にあるチェックボックスをオンにします。
2. **アクション**/**コンテンツの整理**/**コンテンツをフォルダーにコピー**&#x200B;を選択します。 **フォルダーにコンテンツをコピー**&#x200B;パネルが開き、フォルダーの階層が表示されます。
3. コンテンツを表示する出力先フォルダーを選択します。
4. 「**コンテンツをコピー**」を選択します。 コンテンツライブラリが更新され、コピーされたコンテンツの新しいフォルダパスが表示されます。

>[!NOTE]
>
>プライベートフォルダーからパブリックフォルダー、またはパブリックフォルダーからプライベートフォルダーにコンテンツをコピーすることはできません。
...
## フォルダーを参照してコースにモジュールを追加する

セルフペースモジュールをコースに追加する場合、名前で検索するのではなく、フォルダーの階層を参照して適切なコンテンツファイルを見つけることができます。

1. 作成者としてログインし、左側のナビゲーションで「**コース**」に移動します。

2. 「**追加**」を選択して、新しいコースを作成するか、既存のコースを編集モードで開きます。

3. **モジュール**&#x200B;セクションに移動し、**コンテンツ**/**モジュールの追加**&#x200B;を選択します。

4. **モジュールの種類を選択**&#x200B;ダイアログで、モジュールの種類として&#x200B;**セルフペース**&#x200B;を選択します。 **セルフペースモジュール**&#x200B;ダイアログが開きます。

5. **[フォルダーの参照]**&#x200B;を選択して、フォルダー階層を開きます。

6. フォルダーレベルを移動して、追加するモジュールを見つけます。 コンテンツファイルを選択します。

7. 「**追加**」を選択します。 モジュールがコースに追加されます。

8. 残りのコースフィールドをすべて入力し、「**保存**」を選択します。
 ...
 <tbody>
  ...
  <tr>
   ...
   <td>
    ...
    <p><b>注意：</b>Captivate、Presenter Video Express または Presenter の HTML コンテンツのみ編集できます。</p></td>
  </tr>
 </tbody>
</table>
...
コンテンツの一意のIDと有効期限は、コンテンツグループのすべての言語バージョンに適用され、言語に関係なく、すべてのユーザーに一貫したエクスペリエンスを提供します。 作成者はコンテンツの一意のIDを使用して特定のコンテンツをすばやく検索し、トレーニングマテリアルの管理と更新を簡単に行うことができます。
...
Adobe Learning Managerのコンテンツライブラリでは、アップロードしたコンテンツのバージョンも保持されます。 PowerPoint プレゼンテーションなどのコンテンツを変更し、そのプレゼンテーションをライブラリに再度アップロードすると、バージョン番号が 1 つ大きくなります。 これにより、コンテンツの変更内容が追跡しやすくなります。 コンテンツの新しいバージョンは、すべての学習オブジェクトに同時に適用することも、コースごとに個別に適用することもできます。
+++

+++コースの詳細を別の言語で編集するにはどうすればよいですか？
