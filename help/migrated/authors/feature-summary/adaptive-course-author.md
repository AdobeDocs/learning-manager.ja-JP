...
* 複数のユーザーグループに属する学習者は、最も制限の厳しい結果を得ることができます。いずれかのグループがモジュールを必須にすると、その学習者はそのグループのモジュールを必須とします。
...
* シートは、新しいインスタンスで完了していない表示モジュールに対してのみ使用されます。
* 新しいインスタンスに表示されているモジュールに使用可能な席がない場合、学習者はこれらのセッションのキャンセル待ちになります。
