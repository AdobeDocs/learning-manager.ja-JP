...
アダプティブコースを含むカタログがピアアカウントの外部で共有される場合、次の動作が適用されます。

* **アダプティブコースが直接共有されています：**&#x200B;アダプティブコースは、共有カタログから除外されます。 受信者のアカウントには表示されません。
* **学習パスまたは資格認定に含まれる適応コース：**&#x200B;適応コースを含む資格認定または資格認定が共有されている場合、その資格認定または資格認定そのものが受信側のアカウントにコピーされます。 その中の適応コースは&#x200B;**通常コース**&#x200B;としてコピーされます。すべての表示ルールおよび完了ルールを含む適応設定はコピーされません。 受信側のアカウントの作成者には、すべての学習者に対してすべてのモジュールが表示される通常のコースとしてコースが表示されます。
* **適応コースが前提条件として設定されています：**&#x200B;適応コースが、共有されている標準コース、学習パス、または資格認定の前提条件として設定されている場合、前提条件の関係は受信側のアカウントに反映されません。 親コースまたはLOが、前提条件なしで受信側アカウントに到着します。

>[!NOTE]
>
>適応構成はカタログ共有中にコピーされないため、カタログを外部で共有する前に、すべての前提条件の関係およびLP/資格認定構造を確認してください。 受信側のアカウントの学習者は、ソースアカウントの学習者と同じように適応する動作にはなりません。
...
>[!NOTE]
>
>**順番付き学習パス：**&#x200B;順番付きコースが順番付き学習パスに含まれている場合、順番付きコースに表示されるモジュールがない学習者は、順番付きコースを完了できません。 これにより、注文した学習パス内の後続のすべての項目がアクセスできなくなります。 学習パスに登録されているすべての学習者が、少なくとも1つのユーザーグループに属していることを確認してください。これにより、パス内のすべての適応コースに、少なくとも1つのモジュールが表示されます。

>[!NOTE]
>
>**通常の学習パス – 自動登録解除：**&#x200B;ユーザーグループの変更により通常の学習パス内の適応コースから学習者が自動登録解除されると、親学習パスは登録状態のままになります。 学習パスの登録は自動解除されません。 アダプティブコースにアクセスできなくなっても、学習者にはトランスクリプトに登録されている学習パスが表示されます。 ユースケースにより、適応コースの登録時に学習パスの登録も解除する必要がある場合は、通常の学習パスの代わりに&#x200B;**適応学習パス**&#x200B;を使用します。
...
通常のコースでは、インストラクターは&#x200B;**キャンセル待ちリストの上限**&#x200B;を設定できます。これは、セッションのキャンセル待ちリストに追加できる学習者の数の上限です。

アダプティブコースでは、インストラクターアプリで&#x200B;**キャンセル待ちの制限**&#x200B;設定が無効になっているため、設定できません。 アダプティブコースのセッションでキャンセル待ちできる学習者の数に制限はありません。 セッションが満席のときに登録を試みた学習者は、制限なくキャンセル待ちになります。
...
>[!NOTE]
>
>**Flex学習パスのビヘイビアー：**&#x200B;適応コースがFlex学習パスの一部である場合、キャンセル待ちのビヘイビアーは直接登録のビヘイビアーとは異なります。 学習者がFlex LP内でアダプティブコースのインスタンスを選択し、そのインスタンスに使用できる席がない場合、そのインスタンスのキャンセル待ちになります。 このシナリオのキャンセル待ち学習者の情報は、**管理者/[アダプティブコース]/キャンセル待ち**&#x200B;からのみ表示できます。**管理者/学習パス**&#x200B;には表示されません。 Flex LPを通じてキャンセル待ちになった学習者を管理するには、アダプティブコースの独自の「キャンセル待ち」タブを確認します。

Flexラーニングパスの一部であるアダプティブコースでセッションの&#x200B;**出席レポートPDF**&#x200B;をダウンロードすると、キャンセル待ちの学習者がPDFの&#x200B;**アクティブ**&#x200B;セクションに表示されます。 これは、学習パスインターフェイスに個別のキャンセル待ちセクションがないためです。 **管理者/[適応コース]/キャンセル待ち**&#x200B;を使用して、キャンセル待ちの学習者を特定し、出席を記録する前に確認済みの出席者と区別します。
