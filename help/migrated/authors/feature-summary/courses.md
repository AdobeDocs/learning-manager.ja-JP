...
1. 「**[!UICONTROL 保存]**」をクリックして変更内容を保存し、 「**[!UICONTROL パブリッシュ]**」をクリックしてコースをパブリッシュします。

## コースの作成 - 高度なワークフロー {#createacourseadvancedworkflow}

1. Adobe Learning Manager に作成者としてログインします。これは作成者のみがコースを作成する権限を持つためです。 ホームページで「**[!UICONTROL コースの作成]**」を選択します。 コースカタログページが開き、コースのリストが表示されます。
2. 右上の「**追加**」を選択します。 新しいコースを作成するためのページが開きます。
3. コースの名前と簡単な説明を入力します。 この説明はコースカードに表示されます。 この説明は 140 文字以下にする必要があります。 次に、「コースの詳細」ページに表示される、コースの詳細な説明を入力します。 この説明は 1500 文字以下にする必要があります。
4. 別の言語でコースを作成する場合は、ページの左上隅に表示されている「新しい言語を追加」をクリックします。 コースで使用する 1 つ以上の言語を選択します。 「**[!UICONTROL 保存]**」をクリックします。 詳細については、「[各種言語のコンテンツの追加](/help/migrated/authors/feature-summary/content-library.md)」を参照してください。
5. **以下の手順で、コースの設定を変更します**。

   a. 「コース設定」ページでコースのスキルを選択し、 「スキル」ドロップダウンリストで、必要なスキルを選択します。 次に、「レベル」ドロップダウンリストで、必要なレベルを選択します。
   b. コースのスキルとレベルを選択し、スキルの単位を設定します。 必要に応じて、さらにスキルを追加します。
   c. 必要に応じて、コースにカスタム準拠ラベルを追加します。 [コース/学習パス/資格認定への準拠ラベルの追加](/help/migrated/authors/feature-summary/courses.md#add-compliance-labels-to-courselearning-pathcertification)を参照してください。
   d. 「**登録タイプ**」ドロップダウンリストで、登録のタイプを選択します。
...
6. コースの価格を設定するか、無料にするかを選択します。 コースを有料にする場合は、オプションを選択します **[!UICONTROL 有料]**&#x200B;を選択し、価格を指定します。 価格は、学習者のコースカードとコース概要ページに表示されます。

   >[!NOTE]
   >
   >これは、Adobe Commerceコネクタが設定されている場合にのみ有効になります。

7. 学習者が自分自身でコースの登録を解除できるようにするには、「**学習者は自分で登録解除できます**」チェックボックスを選択します。
...
8. **インスタンス構成**
   ...
   * アクティビティ
   ...
   >[!NOTE]
   >
   >コースの複製中に、複製元のコースで「インスタンス構成」オプションを有効にした場合、複製先のコースではオプションが無効になります。
...
10. 必修コースを選択します。学習者は、これらのコースを完了しない限り、目的のコースを開始することはできません。 「コース」フィールドをクリックし、コースの一覧で前提条件となるコースを選択します。
11. 必修コースを必須にする場合は、「**前提条件を有効にする**&#x200B;**&#x200B;**」チェックボックスを選択します。
12. コースに関連するキーワードをタグとして追加します。 学習者はこのタグを検索することにより、目的のコースを簡単に探すことができます。 これらのタグはすべて、追加したモジュールに基づいて自動的に追加されます。 このコースに他のタグを追加する場合は、そのタグを入力します。
13. コースに関連するキーワードをタグとして追加します。 学習者はこのタグを検索することにより、目的のコースを簡単に探すことができます。 これらのタグはすべて、追加したモジュールに基づいて自動的に追加されます。 このコースに他のタグを追加する場合は、そのタグを入力します。
14. 「自動廃止」フィールドで、コースを廃止する日付を選択します。 管理者は、まず「自動廃止」オプションを有効にする必要があります。
15. 変更を保存するには、[**[!UICONTROL 保存]**]を選択します。 「**[!UICONTROL パブリッシュ]**」をクリックしてコースをパブリッシュします。

## コースの成績表の設定 {#configuregradebookforacourse}
...
* **Gradebook**&#x200B;の切り替えを選択して有効にします。 その下に2つのオプションが表示されます。 両方ともデフォルトでオンになっています。
  * **学習者に成績表を表示：**&#x200B;学習者には、コースプレーヤーに&#x200B;**成績表**&#x200B;タブが表示され、モジュールのスコア、加重内訳、および総合的な結果が示されます。 これをオフにすると、学習者に公開せずに内部で成績を計算できます。
  * **成績評価に影響しないモジュールを含める：**&#x200B;採点不可能なモジュール（PDF、ビデオ、オーディオなど）が成績表に表示されます。 採点不可能なモジュールは、学習者の最終的なスコアには含まれません。
...
1. **[モジュールの追加]**&#x200B;を選択します。
2. モジュールピッカーで、追加するモジュールを選択し、**追加**&#x200B;を選択します。 **コンテンツ**&#x200B;セクションにモジュールが表示されます。 採点可能なモジュール、SCORM、Captivateコンテンツ、AICC、xAPI、ネイティブクイズ、アクティビティモジュール、教室セッション、バーチャルクラスルームセッションでは、**加重値**&#x200B;の入力フィールドが表示されます。 採点不可能なモジュールは、加重列にダッシュが表示されます。
3. 採点可能なモジュールごとに、**加重値**&#x200B;フィールドにパーセント値を入力します。 入力に応じて&#x200B;**合計加重値**&#x200B;のインジケーターが更新されます。保存するには、**100%**&#x200B;に正確に到達する必要があります。

   ![](assets/image_0004.png)

4. 複数の配信タイプを持つモジュールの場合：加重値は、モジュールの&#x200B;**すべて**&#x200B;配信タイプがスコアリングをサポートする場合にのみ割り当てることができます。 いずれかの配信タイプがスコアリングをサポートしていない場合、モジュール全体に重み付けすることはできません。
...
## インスタンスの作成 {#createaninstance}

インスタンスを作成するには、次の操作を行います。

1. **作成者**&#x200B;としてログインします。
2. インスタンスを作成するコースに移動し、コースを開きます。
3. 左側のナビゲーションパネルで&#x200B;**インスタンス**&#x200B;を選択します。 **インスタンス**&#x200B;ページが開きます。 各コースには、デフォルトでインスタンスが設定されています。 このデフォルトのインスタンスは変更することも、別のインスタンスを追加することもできます。 ただし、このデフォルトのインスタンスを削除することはできません。
4. インスタンスを作成するには、コース情報の右上隅に表示されている「**[!UICONTROL 新しいインスタンスを追加]**」をクリックします。 コースの新しいインスタンスが表示されます。
5. インスタンスのプロパティを入力します。

   * **[!UICONTROL インスタンス名]**&#x200B;フィールドに、コースに関連付けるインスタンスの名前を入力します。 その際、一意のインスタンス名を入力してください。
   * **開始日：**&#x200B;これはコースの開始日です。 この開始日より前に、選択した2つの日付に2通のリマインダーメールを学習者に送信できます。 開始日は完了日より前の日付でなければなりません。 開始日を選択すると、インスタンスの&#x200B;**通知アラート**&#x200B;セクションのカレンダーで、この日付が青色で強調表示されます。
     ![](assets/start-date-instance.png)
     ![](assets/start-date-highlighted-instance.png)
   * インスタンスの完了期限を指定します。 学習者は、この日までにコースを完了する必要があります。
   * 他の期限オプションを表示するには、**[!UICONTROL その他のオプションを表示]**&#x200B;を選択します。
...
### 開始日より前に電子メールリマインダーを設定

電子メールリマインダーは、インスタンス情報全体を保存した後にのみ設定できます。 「**保存**」を選択すると、インスタンスはそれ自体を最小化し、編集モードになります。

1. **通知アラート**&#x200B;ボックスを選択します。
2. カレンダーで、最初のリマインダーのインスタンスの開始日より前の日付を選択します。
3. 右側の「**リマインダーを追加**」を選択します。
4. **電子メールテンプレート：**&#x200B;ドロップダウンリストからテンプレートを選択してください
5. **定期的なアイテム：**&#x200B;ドロップダウンリストから定期的なアイテムの種類を選択します。 右側の青いチェックマークを選択して、リマインダー日付を保存します。 リマインダーの日付は個別に保存する必要があることに注意してください。
   ![](assets/start-date-reminder1.png)
6. 2回目のリマインダーとして、コースの開始日より前の別の日付を選択してプロセスを繰り返し、右側の青いチェックマークを選択して、このリマインダーの日付も保存します。
   ![](assets/start-date-reminder2.png)

## インスタンスプロパティの表示 {#viewpropertiesoftheinstance}

1. **モジュール**：コースの作成者が作成したモジュールの数。
2. **セッション：**&#x200B;コース内のバーチャルクラスルームおよびクラスルームのモジュール数
3. **実績：**&#x200B;学習者の実績
4. **通知アラート:**&#x200B;コースの開始日前に2通のリマインダーメールを送信するオプション

### アクション {#actions}

* **コース設定に保存：**&#x200B;このインスタンスの設定を、インスタンスを作成したコースに保存します。
* **既定の設定の適用：** **構成** > **インスタンスの既定**&#x200B;ページで見つかった既定の設定を適用します。
* **インスタンスの廃止：**&#x200B;インスタンスを廃止しました。 インスタンスは利用できなくなります。
* **インスタンスの削除：**&#x200B;特定のインスタンスを削除します。


>[!NOTE]
>
>デフォルトインスタンスまたはデフォルトインスタンスのコピーを削除することはできません。 この場合、「アクション」ドロップダウンリストの下に「インスタンスを削除」オプションは表示されません。

* **キャンセル待ち学習者への通知：**&#x200B;キャンセル待ち学習者に、登録できる別のインスタンスがあることを通知します
* **登録された学習者にメールを送信：**&#x200B;重要なお知らせに関するメールを学習者に送信します
...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td>
    <p>試行間の特定の時間のモジュールをロックするには、[<b>試行間のモジュールをロックする0:0:1の形式：日/時間/分</b>]チェックボックスをオンにします。 モジュールがロックされている場合、指定されているロック時間が経過するまで、学習者はそのモジュールを使用することはできません。 </p>
...
1. 「**[!UICONTROL モジュールを追加]**」をクリックします。 モジュールを追加するための 4 つのオプションが表示されます。 最初のオプションは、セルフペースモジュールを追加します。 モジュールを作成し、Adobe Learning Manager のモジュールライブラリに追加します。 2 番目のオプションは、バーチャルクラスルームを設定します。 3 番目のオプションは、教室モジュールを設定します。4 番目のオプションは、アクティビティモジュールを設定します。
   ...
   **注意：**&#x200B;学習者は Microsoft Teams を使用することが必要です。 ただし、学習者はゲストとして Learning Manager に参加できます。
   ...
   管理者がALMアカウントのライブハブを有効にしている場合は、ライブハブコースも作成できます。 Live Hubを使用してVCセッションを作成するには、[Live Hubセッションの作成](../../getting-started-with-live-hub/create-a-live-hub-session.md)を参照してください
   ...
   **教室モジュール：**&#x200B;このモードでは、学習者は実際に講義に出席し、訓練を受けたインストラクタから指導を受けます。 セッションのタイトルと説明を入力し、セッションの期間を設定します。 また、セッションを実施するためのクラスの場所や、インストラクタを指定することもできます。 変更を保存するには、「**[!UICONTROL 完了]**」をクリックします。
   ...
   >[!NOTE]
   >
   >ここで新しい場所を作成できるかどうかは、管理者が設定した「作成者に場所の作成を許可する」設定によって異なります。 無効になっている場合は、管理者が既に追加した場所のみを検索して選択できます。 詳細については、[教室の場所を追加](../../../migrated/administrators/feature-summary/classroom.md)を参照してください。
   ...
   *アクティビティモジュールの追加*
   ...
   コースでアクティビティモジュールを追加するときに、アクティビティタイプがファイル送信のモジュールと xAPI ベースのモジュールに対して、期間を指定できます。
...
チェックリストは複数の言語で作成できます。

### 複数の言語でのチェックリストの作成 {#createachecklist}
...
1. **「チェックリスト」**&#x200B;オプションを選択すると、いくつかの追加オプションが表示されます。

2. **チェックリストタイプ：**&#x200B;**「はい / いいえ」**&#x200B;または&#x200B;**「1 ～ 5」**&#x200B;のいずれかのオプションを選択します。 「はい / いいえ」を選択した場合は、「はい」または「いいえ」でのみ回答できる質問がチェックリストに表示されます。 「1 ～ 5」を選択した場合は、リッカート尺度のチェックリストが表示され、質問に対して 5 段階で評価されます。

3. 「**設定**」を選択します。 [Checklist] （チェックリスト）ウィンドウが開きます。
4. 質問は必要な数だけ入力します。 他の言語でも同じ質問を入力できます。

   ![](assets/multi-lang-support-for-checklists2.png)
   *追加の言語が利用可能です*

   ![](assets/multi-lang-support-for-checklists3.png)
   *たとえば、質問はフランス語でも入力されます*

   >[!NOTE]
   >
   >ALMでは、これらの追加言語で質問が自動的に翻訳されることはありません。 質問はそれぞれの言語に翻訳しなければなりません。

5. 「**保存**」を選択します。 質問はすべての言語で保存されます。
6. その他の関連するオプションをすべて選択し、[**追加**]を選択します。 コースを利用できるすべての言語でチェックリストが作成されます。
...
チェックリストのコメントを使用すると、作成者は、チェックリストベースの評価中に&#x200B;**コンテキストフィードバック**&#x200B;を有効にすることができます。\
有効にすると、レビュー担当者（インストラクターまたはマネージャー）は、スコアや合格/不合格ステータスを超えて評価の結果を説明するコメントを追加できます。

この機能により、チェックリストを評価専用のツールから&#x200B;**フィードバック主導型の学習メカニズム**&#x200B;に変換し、学習者の理解、透明性、およびフォローアップのアクションを向上させることができます。

これを使用して、作成者は次の操作を実行できます。

* チェックリストのレビューアーのコメントを有効にする
* 学習者に対するコメントの表示を制御
* レビューアーの名前を学習者に表示するかどうかを制御します
* レビュー担当者のコメントが学習者のトランスクリプトとレポートにキャプチャされるようにします。

#### ユースケース

#### 業績評価

実際の評価またはロールベースの評価で確認された長所、ギャップ、および改善領域に関する明確なフィードバックを提供する。

#### インストラクターによるトレーニングとVILTトレーニング

学習者がチェックリストベースの評価に合格または不合格した理由をインストラクターが説明できるようにします。

#### コンプライアンスと監査のシナリオ

チェックリスト評価の決定を説明する監査可能な証跡を維持します。

#### 複数のレビューアー環境

学習者が評価したユーザーを特定し、明確にするためのフォローアップを支援します。

#### チェックリストコメントを有効にする

1. 既存のコースを開くか、新しいコースを作成します。
2. チェックリストモジュールを追加するか、既存のチェックリストモジュールを開いて編集します。
3. チェックリスト設定パネルで：
   1. **レビュー担当者のコメント**&#x200B;を選択します。 チェックリストを確認しながら、レビュー担当者はコメントやフィードバックを追加できます。 公開するとこの設定を変更できなくなります。
   2. （オプション）「**学習者にレビューアー名を表示**」を選択します。 これにより、学習者はチェックリストモジュールをレビューしたレビュー担当者の名前を表示できます。
4. チェックリストの構成を保存します。
5. コースをPublishして、評価にチェックリストを利用できるようにします。

チェックリストに、学習者の評価中のレビュー担当者のコメントが含まれるようになりました。

#### 評価中に何が起こるか

作成者は評価を実行しませんが、設定がどのように使用されるかを理解しておくと便利です。

* 学習者の評価中に、レビュー担当者に&#x200B;**備考フィールド**&#x200B;が表示されます
* レビュー担当者は提出中にコンテキストコメントを追加できます
* 有効になっている場合：
  * 学習者、「レビュー担当者のコメント」を参照
  * 学習者にはレビュー担当者の名前が表示されます

#### 学習者エクスペリエンス（有効な場合）

* 学習者が完了済みチェックリストを開いた場合：
* チェックリストの結果（合格/不合格またはスコア）が表示されます
* レビュー担当者のコメントが表示されます（有効な場合）
* レビューアーの名前が表示されます（有効な場合）

これにより、学習者は評価の結果と次のステップを明確に理解できます。

#### 作成者向けのキーノート

* チェックリストのコメントは&#x200B;**オプション**&#x200B;であり、明示的に有効にする必要があります
* コメントとレビューアーIDの表示は&#x200B;**完全に作成者が制御します**
* 対象：
  * 単一または複数のレビュー担当者
  * スコアベース、はい/いいえ、およびスケールされたチェックリスト
* 作成者によって更新されない限り、既存のチェックリストに影響はありません

### 多言語チェックリストの作成

チェックリストの複数言語サポートにより、作成者は&#x200B;**複数の言語で利用可能な質問を含む単一のチェックリストモジュール**&#x200B;を作成できます。\
有効にすると、レビュー担当者は&#x200B;**選択したコンテンツ言語**&#x200B;でチェックリストの質問を自動的に表示し、グローバルチーム全体で一貫性のあるローカライズされた評価を行うことができます。

これにより、**言語ごとに個別のチェックリスト**&#x200B;を作成および管理する必要がなくなり、重複が減り、コースのメンテナンスが簡素化されます。

多言語チェックリストのサポートにより、作成者は次のことができます。

* 同じチェックリスト内の複数の言語にチェックリストの質問を追加する
* チェックリストの質問で言語の一致を維持する
* レビュー担当者に、コンテンツ言語に自動的にローカライズされたチェックリストの質問を表示させる
* 選択した言語でチェックリストレポートをダウンロード

#### ユースケース

#### グローバルな要員評価

地域全体のマネージャーとインストラクターが、ローカライズされたチェックリストの質問を使用して、学習者を評価できるようにします。

#### 地域間で標準化された評価

地域ごとに複数の言語をサポートしながら、単一の評価構造を維持します。

#### コンプライアンスと運用トレーニング

評価者が母国語でチェックリストの基準を明確に理解し、解釈エラーを減らすようにすること。

#### コンテンツの重複を削減

異なる言語用に複数のチェックリストモジュールを作成して管理することは避けてください。

#### 多言語チェックリストの作成方法

1. 既存のコースを開くか、新しいコースを作成します。
2. チェックリストモジュールを追加します。
3. コース設定で、必要な&#x200B;**コンテンツ言語**&#x200B;を追加します（例：英語、フランス語、ドイツ語）。
4. コース設定を保存します。

#### 複数の言語でチェックリストの質問を構成する方法

1. チェックリストモジュール設定を開きます。
2. **質問の構成**&#x200B;セクションに移動します。
3. 使用可能な言語を切り替えるには、**言語タブ**&#x200B;を使用します。
4. 各言語について：
   * 各チェックリスト質問の翻訳版を追加する
   * 言語間で同じ意味および評価意図を維持する
5. 必要に応じて、チェックリスト基準（「はい/いいえ」、「スケールベース」または「スコアベース」）を定義します。

各言語バージョンは、個別のモジュールではなく、同じチェックリストを表します。

チェックリスト設定を保存し、コースをパブリッシュします。 チェックリストは、設定されたすべての言語で使用できるようになりました。

#### レビュアーの経験

* レビュー担当者には、**選択したコンテンツ言語**&#x200B;でチェックリストの質問が表示されます
* 評価中に手動で言語を切り替える必要はありません
* レビュー担当者がコンテンツ言語を変更すると、チェックリストの質問は自動的に適応します

### チェックリスト質問への重みの割り当て

チェックリストの質問の重み付けにより、作成者は&#x200B;**スコアベースのチェックリスト**&#x200B;を作成できます。個々のチェックリストの質問には、**異なる最大スコア**&#x200B;を含めることができます。

作成者は、すべてのチェックリスト項目を平等に扱う代わりに、重み付けされた質問を定義し、学習者の合格に必要な&#x200B;**最小スコア**&#x200B;を設定できます。

この機能により、特に&#x200B;**パフォーマンスベース、評価ベース、およびコンプライアンス指向のトレーニング**&#x200B;に対して、より正確で柔軟な評価が可能になります。

チェックリストの質問の重要度を使用して、作成者は次の操作を実行できます。

* **スコアベースのチェックリストの種類**&#x200B;の作成
* 個々のチェックリストの質問に異なる&#x200B;**最大スコア**&#x200B;を割り当てる
* チェックリストに合格するために必要な&#x200B;**最小スコア**&#x200B;を定義します
* 拡張スコアリング尺度を使用（最大10の範囲を設定可能）
* レポートの達成スコアと最大スコアの表示
...
単純な「はい/いいえ」の結果ではなく、詳細なスコア基準を使用して学習者を評価します。

#### SOPおよび運用トレーニング

重要な手順には高いウエイトを割り当て、重要でないタスクには低いウエイトを割り当てます。

#### コンプライアンスと認定の準備状況

合格する前に、学習者が定義された最小コンピテンスしきい値を満たしていることを確認します。

#### 技能・性能ベンチマーク

部分的な完了がスコア全体に貢献している構造化評価をサポートします。

#### 重み付きチェックリストの作成方法

1. 既存のコースを開くか、新しいコースを作成します。
2. チェックリストモジュールを追加するか、既存のチェックリストモジュールを開いて編集します。
3. チェックリスト構成パネルで、**カスタムスコアリング**&#x200B;チェックリストの種類を選択します。
4. これにより、質問ごとのスコアリングと合計スコアの計算が可能になります。
5. 必要に応じて、チェックリストの質問を追加します。
6. 質問ごとに、次の手順を実行します。
   * **最大スコア**&#x200B;を定義します（例： 5, 10, 20）
7. 質問のスコアに、相対的な重要度が反映されるようにします。

質問ごとに、同じチェックリスト内で異なる最大スコアを持つことができます。

#### 合格条件の定義

1. チェックリストに合格するために必要な&#x200B;**最小スコア**&#x200B;を指定します。 システムは、質問の重み付けに基づいて可能な合計スコアを自動的に計算します。
2. チェックリスト設定を保存し、コースをパブリッシュします。

インストラクターの評価用に加重スコアリングを使用することで、チェックリストが有効になりました。

#### レビュアーの経験

* レビュー担当者は、各チェックリストの質問を&#x200B;**最大スコア**&#x200B;で確認できます
* レビュー担当者は、評価中に質問ごとにスコアを割り当てます
* 計算は次の通りです。
  * 合計達成スコア
  * 定義された条件に基づく合格または不合格ステータス

#### 学習経験

* 学習者には、チェックリスト完了ステータス（合格/不合格）が表示されます。
* 有効にすると、学習者には次の情報も表示されます。
  * レビュー担当者のコメント
  * レビューアー名
* スコアリング・ロジックは複数の評価間で一貫して適用されます

#### 作成者向けの注意

* 加重値チェックリストは、**正式な評価**&#x200B;に最適です
* アウトカムに偏りがないよう、質問のスコアは慎重にキャリブレーションすべきである
* 対象：
  * 単一または複数のレビュー担当者
  * レビュー担当者のコメント（有効な場合）
* 既存のチェックリストは、明示的に更新されない限り影響を受けません
...
7. 「**[!UICONTROL インストラクターに成功のマークを許可]**」オプションを選択します。
   ...
   ![「インストラクターに成功のマークを許可しますか？」 チェックボックスがハイライト表示され、作成者はモジュールの学習者の成功ステータスをインストラクターで記録できる](/help/migrated/authors/feature-summary/assets/allow-instructor-mark-success.png)
...
* **削除済み** - Adobe Learning Managerアプリケーションからコースが完全に削除された段階です。 作成者のみが、「草案」状態と「撤回済み」状態のコースのみを削除できます。
...
1. 管理モードで、左側のペインの「**[!UICONTROL コース]**」をクリックします。 このページでは、Learning Manager アカウントのすべての作成者が作成したすべてのコースを表示できます。
...
1. 「学習者を含む」フィールドに学習者の名前を入力し、学習者を選択します。 ここでユーザーグループを追加することもできます。 すべての学習者を Learning Manager アカウントに登録する場合は、すべての入力を開始します。 また、学習者をチームに登録することもできます。
...
1. 新しく作成されたカタログのオプションを使用して、追加できる学習オブジェクトのタイプを選択します（学習プログラム、資格認定、コースなど）。
...
**前提条件：**&#x200B;管理者が、管理アプリで&#x200B;**設定／一般**&#x200B;と進み&#x200B;**「複数回の試行」**&#x200B;オプションを有効にしている必要があります。
