...
1. **以下の手順で、コースの設定を変更します**。
   ...
   * **マネージャーにより承認済み**：これは、マネージャーが承認を行うコースです。学習者はこれらのコースにサインアップできますが、マネージャーの承認なしにこれらのタイプのコースに直接登録されることはありません。 学習者がこのコースに申し込むと、通知がマネージャーに送信されます。この申し込みをマネージャーが承認すると、このコースが登録済みコースとして表示されます。
   * **セルフ登録済み**：このタイプのコースの場合、学習者自身が直接登録することができます。

1. 「**[!UICONTROL 保存]**」をクリックして変更内容を保存し、コースを公開するには、 **[!UICONTROL 公開]**&#x200B;をクリックします。
...
1. **以下の手順で、コースの設定を変更します**。
   ...
   1. 「コース設定」ページでコースのスキルを選択し、 「スキル」ドロップダウンリストで、必要なスキルを選択します。 次に、「レベル」ドロップダウンリストで、必要なレベルを選択します。
   1. コースのスキルとレベルを選択し、スキルの単位を設定します。必要に応じて、さらにスキルを追加します。
   1. 必要に応じて、カスタムコンプライアンスラベルをコースに追加します。 [コース/ラーニングパス/認定資格へのコンプライアンスラベルの追加](/help/migrated/authors/feature-summary/courses.md#add-compliance-labels-to-courselearning-pathcertification)を参照してください。
   1. 「**登録タイプ**」ドロップダウンリストで、登録のタイプを選択します。
...
1. コースの価格を設定するか、無料にするかを選択します。 コースを有料にする場合は、オプションを選択します **[!UICONTROL 有料]**&#x200B;を選択し、価格を指定します。 価格は、学習者のコースカードとコース概要ページに表示されます。
   ...
   注意: これは、アドビコマースコネクタが設定されている場合にのみ有効になります。

1. 学習者が自分自身でコースの登録を解除できるようにするには、「**学習者は自分で登録解除できます**」チェックボックスを選択します。

1. **インスタンス構成**

   このオプションを有効にすると、進行中の状態にある学習者は、他のインスタンスにアクセスしてそこに登録できます。 これにより、学習者は前のインスタンスの進行状況を保持できます。
   ...
   * 混合
   ...
   注:コースの複製中に、ソースコースで[インスタンス設定]オプションを有効にした場合、オプションは宛先コースで無効のままになります。

   **インスタンスの切り替えはサポートされていません**:
...
1. **複数の登録**
   ...
   これを使用して、1つまたは異なる期間に複数のコースインスタンスに学習者を登録できます。

   トグル **複数登録** を有効にして、学習者のさまざまなコース登録を切り替えます。 「インスタンスの切り替え」を有効にしている場合は、「複数登録」を使用できません。

1. 必修コースを選択します。学習者は、これらのコースを完了しない限り、目的のコースを開始することはできません。「コース」フィールドをクリックし、コースの一覧で前提条件となるコースを選択します。
1. 前提条件のコースを必須にする場合は、 **有効化** **前提条件** チェックボックスをオンにします。
...
1. コースに関連するキーワードをタグとして追加します。学習者はこのタグを検索することにより、目的のコースを簡単に探すことができます。これらのタグはすべて、追加したモジュールに基づいて自動的に追加されます。このコースに他のタグを追加する場合は、そのタグを入力します。
1. [自動廃止]フィールドで、コースが廃止される日付を選択します。 管理者は、まず「自動廃止」オプションを有効にする必要があります。
1. 「**[!UICONTROL 保存]**」をクリックして変更内容を保存し、コースを公開するには、 **[!UICONTROL 公開]**&#x200B;をクリックします。

### コース/ラーニングパス/認定資格へのコンプライアンスラベルの追加 {#add-custom-compliance-label}

コンプライアンスラベルをコースに追加するには、次の手順に従います。

1. 作成者アプリで、 **[!UICONTROL コース]**/**[!UICONTROL ラーニング パス]**/**[!UICONTROL 認定資格]** に移動し、 **[!UICONTROL 追加]** を選択します。
1. 名前と、説明、スキルなどのその他の詳細を入力します。
1. **[!UICONTROL カスタム コンプライアンス]**&#x200B;テキスト ボックスに、コンプライアンス ラベルを入力して選択します。
   ...
   >[!IMPORTANT]
   >
   >カスタムコンプライアンスを追加する場合は、必ずコースの期限を設定してください。

1. コース/ラーニングパス/認定を保存して公開します。現在、コース/ラーニングパス/認定はコンプライアンスタイプと見なされます。 管理者は、このコースをコンプライアンスダッシュボードに追加し、マネージャーと共有して進捗状況を追跡できます

>[!NOTE]
>
>作成者は、コンプライアンスラベルを編集して、既存のコース/ラーニングパス/認定に追加することもできます。

## ゲーミフィケーションポイント

ゲーミフィケーションポイントはコースレベルとコースインスタンスレベルで割り当てることができます。 これにより、さまざまなコースやインスタンスに対してポイントを付与できます。 学習者は、特定のコースを受講したり、特定のコースインスタンスを他のコースよりも優先したりするように動機付けられます。
...
1. [コースレベルの設定を使用]を選択すると、次のオプションが表示されます。
...
作成者として、 **[!UICONTROL ラーニング パス]** > **[!UICONTROL 設定]** を選択します。 **[!UICONTROL 編集]**&#x200B;をクリックします。

[ **[!UICONTROL リソース]** セクションで、[ラーニング パス レベルで集計された構成コース リソースを表示する] チェック ボックスをオンにすると、コース レベルに存在するリソースがラーニング パス レベルに表示されるかどうかが表示されます。

>[!NOTE]
>
>ラーニング パスの [設定] ページで、管理者はこのオプションを有効にして、ラーニング パス レベルに表示されるコース レベルに存在するリソースを表示することもできます。

## スケジュール管理アシスタント

インストラクターと教室の予約状況で発生した競合を管理します。 インストラクターをコースに割り当てる前に、インストラクターが何時と日付に対応しているかを知りたい場合は、スケジュールアシスタントを使用します。

コースの作成中に、VCまたはCRコースの場合は、[スケジュールアシスタント]をクリックします。

![スケジュール アシスタントの選択](assets/scheduling-assistant.png)

*スケジュール アシスタントを起動する*

[スケジュール アシスタント] ウィンドウが起動します。

![スケジューリングアシスタント画面](assets/scheduling-assistant-window.png)
...
![講師を名前で検索](assets/search-instructor.png)

*インストラクターを検索する*

複数のインストラクターを選択できますが、割り当てられるインストラクターは一度に 1 人だけです。時間競合ウィンドウで、選択した時間が強調表示されます。 インストラクターの近くに十字アイコンが表示され、これをクリックしてインストラクターを削除します。

![複数のインストラクターを選択する](assets/busy-times.png)
...
スキルは、スキルレベルではなく、部分的または完全なスキル名でのみ検索できます。

インストラクターの名前、場所、人数制限をアシスタントに入力します。 

また、インストラクター検索ボックスの右側にあるフィルターアイコンをクリックすると表示されるスキルを検索することもできます。 次のスクリーンショットでは、そのボタンの場所が分かります。

![インストラクターのスキルを入力する](assets/scheduling-assistant-instructor-skill.png)
...
これは、[コースまたはインスタンス]ページのスケジュールアシスタントに適用されます。

![スケジューリングアシスタント](assets/scheduling-assistant-2.png)
...
スケジュール アシスタントは、[インスタンス] ページで管理者やカスタム管理者/作成者にも使用できます。

![[インスタンス] ページからのスケジュール アシスタント](assets/instances-scheduling.png)

*[インスタンス]ページからインストラクターをスケジュールする*

### 場所の検索

モジュールとスケジュール アシスタント ページの両方でクラスルーム名と場所地域名の両方を指定することで、場所を検索できます。

## リッチテキスト書式設定

コース、学習プログラム、資格認定や作業計画書を作成する際に、作成者はテキストや画像など様々な種類のコンテンツを入力したり、様々なテキスト書式設定オプションを適用したりできます。

コースを作成すると、[コース概要]フィールドにリッチテキストエディタが表示されます。 コンテンツの書式設定、画像の追加、ハイパーリンクの追加などを行うことができます。
...
添付された CSS のスタイルシートには、Learning Manager で適用されるスタイルが含まれています。このスタイルは、さまざまユースケースを念頭において調整されています。 自分の命名規則やビルドシステムに沿って、添付された CSS ファイルをダウンロードして Web アプリに読み込みます。 定義されたCSSクラスはql-editorクラスの下で名前空間化され、既存のスタイルに干渉しません。
...
お客様がヘッドレスインターフェイスを構築する際は、開発中のカスタムユーザーインターフェイスで学習オブジェクトを表示する必要があります。 これを行うには、通常、公開されている [GET /learningObjects](https://learningmanagereu.adobe.com/docs/primeapi/v2/#!/learning_object/get_learningObjects) APIを使用します。 ラーニングマネージャーで概要フィールドの「リッチテキスト」のキャプチャがサポートされるようになったため、API応答のラーニングオブジェクトのデータモデルも同じことを公開します。 以下の API 応答のモデルのフラグメントにある &quot;richTextOverview&quot; という名前のフィールドを参照してください。 また、以前に公開されたフィールド (&quot;概要&quot;) は、下位互換性のために変更されていないことに注意してください。
...
*インタラクティブな e-ラーニングモジュールの複数の答案を構成する*
...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td>
    <p>特定のモジュールについて、試行回数の制限値を設定することができます。制限値を設定しなかった場合は、試行回数が無制限になります。<span style="font-size: 0.8125rem;">試行回数の制限値を設定すると、その情報が学習者に対して表示されます。学習者は、「再試行」ボタンをクリックしてモジュールを再試行することを選択できます。</span></p></td>
  </tr>
  <tr>
   <td>
    <p>モジュールが完了して合格したら新しい試行を停止する</p></td>
   <td>
    <p>学習者が新しい答案オプションを選択できないようにするタイミングを設定するには、「モジュールが完了または合格したら新しい答案を停止する」チェックボックスを有効にします。 「再試行」オプションは、モジュールが正常に完了すると、学習者ビューから削除されます。</p></td>
  </tr>
  <tr>
   <td>
    <p>試行間のモジュールのロック 0:0:1 形式: 日/時間/分</p></td>
   <td>
    <p>「<b>試行間でモジュールをロックする 0:0:1 形式: 日/時間/分</b>」チェックボックスをオンにすることで、試行間の特定の時間モジュールをロックできます。 モジュールがロックされている場合、学習者は指定されたロック時間が経過するまでモジュールにアクセスできません。 </p>
    <p>「<b>プレイヤーのクローズ</b>」または「<b>完了</b>」チェックボックスを選択することで、答案の終了基準を定義できます。</p></td>
  </tr>
  <tr>
   <td>
    <p>プレーヤーを閉じる</p></td>
  ...
  <tr>
   ...
   <td>
    <p>作成者は、「<b>モジュールを完了するための制限時間を設定する</b>」チェックボックスを有効にすることで、モジュールを完了するための時間制限を設定できます。</p>
...
1. 「**[!UICONTROL モジュールを追加]**」をクリックします。モジュールを追加するための 4 つのオプションが表示されます。最初のオプションは、セルフペースモジュールを追加します。モジュールを作成し、Adobe Learning Manager のモジュールライブラリに追加します。2 番目のオプションは、バーチャルクラスルームを設定します。3 番目のオプションは、教室モジュールを設定します。4 番目のオプションは、アクティビティモジュールを設定します。
   ...
   *VC モジュールの追加*
   ...
   会議の開催者に **はい** を選択した場合は、開催者の名前を入力する必要があります。 名前を入力して、主催者を選択します。
   ...
   *クラスルームモジュールを追加する*
   ...
   コースを作成するときは、[仮想教室の構成] ダイアログ ボックスで、作成した Microsoft Teams 接続に会議システムを設定します。 イベントの会議主催者を指定するかどうかを選択します。

   会議の開催者に [はい] を選択した場合は、開催者の名前を入力する必要があります。 主催者の名前を入力して、主催者を選択します。

   **ロビーバイパス**

   * [はい] を選択すると、すべての学習者がミーティングに参加できます。
   * [いいえ] を選択すると、学習者が会議に参加することを許可または禁止する要求が開催者に送信されます。

   **注:** 学習者がゲストとしてMicrosoft Teamsに参加したい場合は、メールアドレスを入力する必要があります。 電子メールはラーニングマネージャに存在する必要があります。
   ...
   *アクティビティ モジュールを追加する*
   ...
   アクティビティタイプのファイル送信およびxAPIベースのモジュールのコースにアクティビティモジュールを追加するときに、期間を指定できます。
...
1. コースのカバー画像やバナー画像を追加します。 カタログは管理者によって作成されます。詳細については、「[カタログ](/help/migrated/administrators/feature-summary/catalogs.md)」を参照してください。
   ...
   * **表紙画像:** 300 ピクセル x 300 ピクセル
   * **バナー画像：** 1600 px x 140 px

1. ページ右上隅の「**[!UICONTROL 保存]**」をクリックします。

#### アクティビティモジュールにHTMLリンクを追加する

作成者は、アクティビティ モジュールに HTML リンクを追加し、完了条件を設定できます。 HTML リンクを追加し、完了条件を設定するには、次の手順を実行します。

1. オーサーアプリケーションで、ホームページの **[!UICONTROL コースを作成]** を選択します。
1. **[!UICONTROL コースカタログ]**&#x200B;画面で&#x200B;**[!UICONTROL 追加]**&#x200B;を選択します
1. コースの名前と説明を入力します。
1. **[!UICONTROL モジュール]**&#x200B;オプションで、**[!UICONTROL モジュールの追加]** > **[!UICONTROL アクティビティモジュール]**&#x200B;を選択します。
1. **[!UICONTROL アクティビティ モジュール]** プロンプトで、名前と説明を入力します。
1. **[!UICONTROL タイプ]**&#x200B;として&#x200B;**[!UICONTROL 外部 URL]** を選択します。
1. 「 **[!UICONTROL 完了基準]** オプションから次のいずれかのオプションを選択します。
   * **[!UICONTROL 学習者が完了をマーク]**: 学習者は、Fluidic Player でコースを完了としてマークするオプションがあります。
   * **[!UICONTROL コンテンツの起動時]**: 学習者がコースを起動すると、コースは自動的に完了としてマークされます。

   ![](assets/completion-criteria-activity-module.png)
   _完了基準_

1. **[!UICONTROL 追加]**&#x200B;を選択してコースを公開します。

## チェックリスト {#create-checklist}

評価は、どの LMS でも重要な要素です。 オンライン評価は、トピックに対する学習者の理解を評価するための主要な方法の1つです。 しかし、多くの場合、必要なタスクを実行するのを観察することによって、仕事中に人の理解を評価する必要があります。
...
チェックリストは、作成者のみが作成できる、 アクティビティモジュールの一種です。 アクティビティモジュールの設定中に、作成者は、次に示すように、アクティビティを **チェックリスト**&#x200B;として選択できます。
...
また、コメントやメモがある場合は、 **講師へのメモ** テキストフィールドに追加できます。
...
1. 更新内容を使用してコースを再公開するには、[ **[!UICONTROL 再公開]**&#x200B;をクリックします。

## 監視チェックリスト

インストラクターに加えてマネージャーも、チェックリストモジュールを確認できるようになりました。 人物マネージャーに加えて非階層マネージャー（ストアマネージャーや場所マネージャーなど）も、チェックリストを確認して項目を完了させることができます。

コース作成者は、チェックリストモジュールの設定時に[レビュー担当者]セクションでこれらのロールオプションを選択することで、ピープルマネージャと非階層マネージャ(該当する場合)をレビュー担当者として追加できます。 コースインスタンスレベルでこの操作を実行できます。

![マネージャー向けチェックリスト](assets/manager-checklist.png)

*アクティビティモジュールにレビュー担当者を追加する*

「**[!UICONTROL +マネージャ]**」オプションを選択すると、組織階層内の学習者のマネージャが自動的にチェックリストを確認できるようになります。 マネージャー名を個別に検索して追加する必要はありません。

アカウント管理者が「アクティブフィールド」オプションを使用して非階層のマネージャーの役割（場所マネージャーやサイトマネージャーなど）を設定している場合は、それらのマネージャーの役割を選択して有効化することで、チェックリストを確認できます。

マネージャー名を個別に検索して追加する必要はありません。 学習者がチェックリストコースに登録すると、選択したインストラクターに加えてマネージャー / ストアマネージャーにも、レビューに関する通知が自動的に送信されます。 このワークフローにより、作成者は個々のマネージャーの名前を簡単に言及しないようにします。

上記のサンプルスクリーンショットでは、「**[!UICONTROL +ストアマネージャー]**」オプションを選択すると、学習者に合わせた非階層マネージャーが自動的にチェックリストを確認できるようになります。 ここでの「store」は、管理者が定義したアクティブなフィールドに置き換えられることに注意してください。
...
![保留中のチェックリストの確認項目を確認する](assets/evaluation-checklist.png)

*保留中のチェックリストの確認項目を確認する*
...
* **廃止** - コースを公開した後、作成者が学習者のコースカタログにコースを表示したくない場合、作成者はコースを廃止状態に移動できます。
* **削除済み** - 削除された状態のコースは、Adobe Learning Manager アプリケーションから完全に削除されたときのことです。 コースを削除できるのは、作成者がドラフト状態または廃止状態のコースのみです。
...
作成者は、コースを並べ替えることで、目的のコースを簡単に探すことができます。**[!UICONTROL 並べ替え]**&#x200B;をクリックし、アルファベット順昇順、アルファベット順降順、コース作成日、コース更新日、およびコースの有効性を選択します。
...
1. 管理モードで、左側のペインの「**[!UICONTROL コース]**」をクリックします。このページでは、ラーニングマネージャアカウントのすべての作成者が作成したすべてのコースを確認できます。
...
1. 学習者を登録ダイアログボックスの右上隅に、「**デフォルトのインスタンス**」オプションが選択されて表示されます。作成者がコースを作成するとすぐに、コースのデフォルトインスタンスが作成されます。

   ![](assets/default-instance.png)

   *コースのデフォルトインスタンスの表示*

1. 「学習者を含む」フィールドに学習者の名前を入力し、学習者を選択します。ここでユーザーグループを追加することもできます。ラーニング マネージャー アカウントにすべての学習者を登録する場合は、「すべて」と入力し始めます。 また、学習者をチームに登録することもできます。
...
1. Captivate で **[!UICONTROL ラーニングマネージャーに公開]** > **[!UICONTROL 「公開]**」をクリックします。
...
作成者はコースの有効性スコアを活用することで、個々の学習者のニーズに合っていないコースを特定し、そのコースの内容を修正できます。コースの有効性を評価することにより、そのコースが学習者にとって有益なものであるかどうかを判断することができます。コースの内容に関する学習者からのフィードバックを組み合わせて、コースの有効性が評価されます。学習者向けのコースクイズの結果と、コースからの学習に基づいて学習者を評価するマネージャーのフィードバック。
...
認定資格と学習プログラムの作成と管理方法については、  [認定資格](/help/migrated/administrators/feature-summary/certifications.md) および  [学習プログラム](/help/migrated/administrators/feature-summary/learning-programs.md)を参照してください。
...
+++コースの「シークマネージャー指名」を削除するにはどうすればよいですか?

次の手順を実行します。

1. 作成者として Learning Manager にログインします。
1. コースを開きます。
1. 左側のペインで、 **[!UICONTROL 設定]** > **[!UICONTROL 編集]**&#x200B;をクリックします。
...
+++すべての学習者がすべてのコースを表示できないようにするにはどうすればよいですか?

カタログを使用すると、学習者に対して表示されるコースを制限することができます。 デフォルトのカタログには、デフォルトでラーニングマネージャに追加されたすべてのコースが含まれています。
...
1. 新しく作成されたカタログオプションでは、学習プログラム、認定、コースなど、追加できるさまざまなタイプの学習を選択できます。
...
1. [カタログ] ページで、 **D[!UICONTROL efault カタログ]**&#x200B;を無効にし、作成したカタログを有効にします。
...
+++学習者はコースを修了した後でもどのように見ることができますか?
...
+++モジュールに複数の試行を設定するにはどうすればよいですか?

**前提条件:**&#x200B;管理者は、管理アプリの&#x200B;**設定>一般**&#x200B;で&#x200B;**複数回の試行**&#x200B;オプションを有効にする必要があります。
...
+++Adobe Learning Manager にアップロードされたコンテンツをダウンロードして、コンテンツを変更できますか?

いいえ、ラーニングマネージャーにアップロードされたコンテンツは公開されたzipファイルであり、ソースファイルではありません。 そのため、コンテンツをダウンロードしても、オーサリングツールでそのコンテンツを編集することはできません。 コンテンツを編集するには、ソースファイルが必要です。
