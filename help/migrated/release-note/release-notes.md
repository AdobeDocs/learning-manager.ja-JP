...
+++アップデート101:Adobe Learning Managerの2025年6月リリース
...
完了したコースのコンテンツモジュールが新しいバージョンに更新されたときに、学習者に白い画面が表示されました。 この問題は、作成者が新しいコンテンツをコースをまだ開始していない学習者のみに適用することを選択した場合にも発生しました。 この問題はユーザーエクスペリエンスに影響を及ぼし、再生に問題が発生したために、コースを再受講しようとした以前の学習者の間で混乱が生じました。

### 既存の動作

作成者がコースコンテンツを新しいバージョン(V2)に更新し、それを「未開始」の学習者にのみ適用した場合、Adobe Learning Managerは完了した学習者のモジュールバージョンをバックグラウンドで更新しました。この更新されたモジュールは学習者に対して有効ではなくなったため、学習者がコンテンツに再度アクセスしようとすると白い画面が表示されました。

### 例：

* 完了した学習者が更新後にコースをもう一度確認しようとすると、白い画面が表示されました。
* 進行中の学習者がコースを完了した後、コンテンツバージョンがサイレントに更新された後、白い画面が表示されました。

### これまでの変更点について

Adobe Learning Managerには、コンテンツを更新する際の作成者に対するより明確なバージョン管理オプションが導入されています。 作成者には、コンテンツのバージョンを更新するときに、次の3つの明確なオプションが表示されるようになりました。
...
**[!UICONTROL 更新が開始されない]**&#x200B;場合、完了した学習者には既存のコンテンツバージョン(V1)が引き続き表示され、予期しない白い画面が表示される問題が解決されます。

* **[!UICONTROL 今すぐ更新]** ：すべての学習者に対してコンテンツの更新を適用します（未開始、進行中、完了済みの学習者は新しいコンテンツバージョンに現在移動します）
* **[!UICONTROL 最終的に更新]** ：すべての学習者に対して、コンテンツの更新を最終的に適用します（開始しない場合、完了済みの学習者は新しいコンテンツバージョンに移動します。進行中の学習者は完了後に移動します）
* **[!UICONTROL 未開始の更新]** ：未開始の学習者に対してのみコンテンツの更新を適用します（進行中の学習者および完了した学習者は既存のコンテンツバージョンに残ります）

### ユーザーインターフェイスの変更

| 古いラベル | 新しいオプション |
|---|---|
| すべての学習者にコンテンツの更新を適用 | 今すぐ全学習者を更新：すべての学習者にコンテンツの更新を適用します |
...
+++アップデート89:Adobe Learning Managerの2023年3月リリース
...
Learning Managerでは、管理者が&#x200B;**マネージャーと共有**&#x200B;ワークフローを使用して、マネージャーとトレーニングを共有し、各自の準拠ダッシュボードにトレーニングを追加できるようになっています。 したがって、マネージャーは何らかのアクションを実行する必要はなく、コンプライアンスの追跡をすぐに開始できます。
...
* **レポートの追加**&#x200B;ダイアログでは、**期間**&#x200B;と&#x200B;**フィルター**&#x200B;のセレクターが個別のセクションとして表示され、既定では折りたたまれた状態になっています。 詳細については、「[***レポートの作成***](../administrators/feature-summary/reports.md#report)」を参照してください。
...
* マネージャーが登録を承認していない場合でも、学習カレンダーに状態&#x200B;**登録済みセッション**&#x200B;が表示されました。 マネージャーが登録を承認するまで、正しいステータス&#x200B;**保留中**&#x200B;が学習者に表示されます。

* 特定のケースでは、学習者がコースを完了していても、セッションの学習カレンダーにステータス&#x200B;**登録済み**&#x200B;が表示されていました。
...
他の掲示板メンバーが投稿を表示して操作を開始できるように、写真、ドキュメント、オーディオファイル、またはビデオファイルを任意の掲示板に共有します。  詳細については、[***投稿の共有***](../learners/feature-summary/ipad-android-tablet-users.md#socialmobile)&#x200B;を参照してください。
...
ソーシャル学習では、学習者が投稿したコンテンツを、**キュレーションなし**&#x200B;と&#x200B;**手動キュレーション**&#x200B;の2つの方法でキュレートできます。 Adobe Learning Manager のこのリリースでは、AI 対応の自動キュレーション機能が提供され、ソーシャル学習が強化されています。コンテンツを投稿すると、コンテンツが分析され、投稿先のスキルに属しているかどうかが判定されます。信頼スコアに基づいて、コンテンツはライブで投稿されるか、手動キュレーションのために送信されます。 詳しくは、*[**&#x200B;自動アシストによるキュレーション&#x200B;**](../administrators/feature-summary/social-learning-configurations-as-an-admin.md#autocuration)**&#x200B;を参照してください。***
...
* マネージャーがオプション&#x200B;**今四半期**&#x200B;を選択した場合、学習目標の登録、進捗状況、および完了の詳細が表示されませんでした。 このリリースでは、これらの詳細が正しく表示されるようになりました。
...
* 共有カタログを介して取得した学習目標の編集機能を完了します。 詳細については、[***共有カタログコントロール***](../administrators/feature-summary/shared-catalog-full-control.md)&#x200B;を参照してください。 この機能を有効にする場合は、Learning Manager サポートにお問い合わせください。
* インストラクターは、保留中のレビューでセッションとモジュールを簡単に識別できるようになりました。 詳細については、[***保留中のレビュー***](../instructors/feature-summary/learners.md#pending)&#x200B;を参照してください。
...
* このリリースの Adobe Learning Manager には、新しい機能と拡張機能が導入されています。カスタムの役割の作成、カタログラベルの追加、ユーザーの消去、タグの管理、学習目標名の変更、Slackの統合、新しいコネクタの統合、xAPIのサポートなど、各種機能を提供します。 新機能と拡張機能の詳細については、[新機能の概要](../whats-new.md#main-pars_text)を参照してください。
...
**プログラムインスタンスが期限切れになった場合の学習プログラムの問題**
...
コースのディスカッションボードを使用して、ピアやインストラクターとやり取りします。 学習者はコースのすべての投稿を確認できます。ただし、削除できるのは自分が入力した投稿のみになります。ディスカッション掲示板の詳細については、[ディスカッションの表示と参加](../learners/feature-summary/courses.md#main-pars_header_1772461149)を参照してください。
...
Adobe Learning Managerでコースの配信タイプを追加できるようになりました。
...
* 学習者の役割で、管理者が学習者のコースの有効性を無効にしている場合でも、**並べ替え**&#x200B;のフィルターオプションの1つとしてコースの有効性が表示されました。 この問題は修正されました。
...
* 資格認定の登録レポートをエクスポートすると、**期限**&#x200B;という新しい列もエクスポートされます。 この列データにより、管理者は学習目標の消費期限を過ぎた学習者を確認できます。
...
* 資格認定の作成時に、管理者が資格認定の有効期間以上の&#x200B;**完了日数**&#x200B;を入力すると、警告メッセージが表示されます。 以前は、管理者に警告メッセージは表示されていませんでした。
...
* 外部ユーザーの登録ページにテキストメッセージが含まれています。**ユーザーは、最初に登録して、以降のログインで使用するユーザー名パスワードを作成する必要があります**。

**ユーザー通知**

* 社外学習者が「コースを再訪」電子メール通知の「**メモを開く**」リンクをクリックするとプレーヤーが開きますが、メモパネルが機能しません。 この問題は修正されました。
* 社外学習者が「コースを再訪」電子メール通知の「**メモを開く**」リンクを使用して事前作業モジュールまたはテストアウトモジュールを開こうとすると、メモの内容が表示されませんでした。 この問題は修正されました。
...
社外学習者が「コースを再訪」電子メール通知の「**メモを開く**」リンクをクリックしても機能しない。 この問題は修正されました。このリンクをクリックすると、ユーザーがLearning Managerにログインしていなくても、プレーヤーが開いてメモが表示されます。
...
管理者は範囲設定を変更することにより、ゲーミフィケーションの範囲を定義できます。類似するプロファイルのユーザー、グループ、または場所に対して、ゲーミフィケーションを選択的に有効にできます。 詳細については、[ゲーミフィケーション](../learners/feature-summary/gamification.md)機能を参照してください。
...
ユーザーを削除した場合、単一ユーザーの追加機能を使用して同じユーザーをアプリケーションに戻すことはできません。ただし、CSV のアップロードプロセスを使用すると、削除したユーザーを追加して戻すことができます。CSV のアップロード機能では、必須フィールドの制限が大幅に変更されています。詳細については、[CSVに関するFAQ](../administrators/add-users-in-bulk.md)を参照してください。
...
草案段階や撤回済み段階にあるコースを削除できるようになりました。詳細については、[コース](../administrators/feature-summary/courses.md)の機能を参照してください。 学習目標が削除された場合、この学習目標のすべてのレポートデータも削除されます。コースが削除され、そのコースが他の学習目標の一部であった場合、適切なメッセージがユーザーに表示されます。
...
Learning Managerで&#x200B;**[!UICONTROL 「ログアウト」]**&#x200B;をクリックすると、Learning Managerアプリケーションのみでなく、Adobe IDからもログアウトできるようになりました。
...
* 管理者の役割で、学習者の氏名が「**学習者**」タブに表示されます。 これまでは学習者の名前（名）のみが表示されていました。
...
* 管理者と作成者の役割でコース、カタログ、学習プログラムにアクセスする際、タブの順序が&#x200B;**公開済み – 下書き – すべて – 廃止済み**&#x200B;に変更されます。 既定の選択は&#x200B;**公開済みです。**
...
* 管理者の役割で、ユーザーが学習者数を入力して「**注文する**」をクリックすると、ユーザーごとの月額および年間のサブスクリプション料金の詳細を含むダイアログが表示されます。
