...
このアプリをダウンロードしてインストールした後、学習者の資格情報を使用してアプリにログインします。ラーニングマネージャーモバイルアプリを開くと、デフォルトでは以下の画面が表示されます。

![](assets/1.png)

*ラーニング マネージャー アプリのホーム ページ*

## ホームページ {#homepage}

イマーシブレイアウトオプションが有効になっているすべてのアカウントには、イマーシブレイアウト構成をサポートする新しいホームページがあります。 ホーム ページは、次のカードの一覧として整理されています。
...
Learning Manager では、モバイルアプリに関するエクスペリエンスについて、フィードバックの提出を促すメッセージが学習者に表示されます。このフィードバックは月に一度表示され、ユーザーがアプリを体験してからフィードバックを提供するのに十分な時間間隔を提供します。
...
既存のユーザーの場合、アプリを最新バージョンに更新した後、初めてプレーヤーを閉じると、フィードバックポップアップが表示されます。

フィードバックの収集は匿名であり、アプリ設定の構成を介して **使用状況データの共有** オプションを有効にしたユーザーに対してのみ表示されることに注意してください。

## バッジのダウンロード {#downloadbadge}

学習者はバッジをPDFおよび画像形式でダウンロードできます。 [ユーザー プロファイル] セクションで、下にスクロールし、いずれかのリンクをクリックしてバッジをダウンロードします。 ダウンロードしたバッジは、デフォルトのデバイスフォルダーに保存されます。 このアプリでは、ダウンロードが完了した直後にフォルダまたはファイルを開くことができます。
...
完全にレスポンシブなモバイルWebエクスペリエンスで、学習者はモバイルアプリをインストールしていない場合でも、デバイスを使用してトレーニングを利用できます。
...
## ALM Web でプロンプトをダウンロード

ユーザーがモバイル デバイスで ALM リンクを開いたり、ALM Web を使用したりすると、エクスペリエンスを向上させるためにアプリをダウンロードするように求めるプロンプトが表示されます。
...
モバイル没入型アプリでサポートされていない機能は これらは：

| **面積** | **サポートされていないシナリオ** |
|---|---|
| 掲示板 | ボード作成、ボード編集、URLコピー、削除、レポート、並べ替え、フィルター |
| 投稿 | 投票タイプ投稿の作成、編集、投稿への複数の添付ファイル、URLのコピー、トップにピン留め |
| コメント/返信 | 編集、正解としてマーク、コメントまたは返信での画像/ビデオ/オーディオのアップロード |
| メディア | 音声ののキャプチャ、音声ファイルのアップロードのみ |
| その他 | 通知、投稿の表示、ソーシャルリーダーボード、フォロー中のユーザー、ユーザープロフィール |

## 学習者による、作業計画書のダウンロードの有効化 {#download-job-aid}

モバイル没入型モードでは、学習者は作業計画書をダウンロードできます。 **設定**&#x200B;をタップしてから&#x200B;**カタログ**&#x200B;をタップします。表示されるリストからジョブエイドをダウンロードします。
...
モバイルイマーシブモードでは、学習者は学習者のトランスクリプトをダウンロードできます。
...
画面下部のブックアイコンの横にあるアイコンをタップします。カタログと関連するすべての学習目標がすぐに表示されます。グリッドレイアウトで表示するか、詳細ビューを選択するかを選択できます。 推奨事項を選択すると、学習履歴に基づいてコースの推奨事項が表示されます。
...
画面の左上隅にあるメニューをタップして ![](assets/hamburger.jpg) プロフィール、通知、アカウント設定を表示します。

このメニューでは、QR コードをスキャンしてコースの出席を記録することもできます。

**QRコードをスキャン**&#x200B;オプションをタップしてコードをスキャンするだけです。

**コースのオフライン受講**

オンライン中にダウンロードしたコースを利用すると、Learning Managerはサーバー呼び出しを行う代わりに、ダウンロードしたコースにアクセスします。
...
* **出席をマーク:** これにより、学習者はQRコードをスキャンして特定のセッションの出席をマークできます。
...
* 掲示板を作成またはフォローすること。
* URL を投稿にコピーすること。
* 投稿をストーリーとして追加するか、お気に入りとして投稿として追加するか、トップにピン留めします。
* ソーシャル リーダー ボードを表示します。
...
投稿を作成したら、[投稿]をタップしてコメントを投稿し、他の学習者が投稿を閲覧できるようにします。
...
* 投稿  ![](assets/prime-like.png)  好きか嫌いか  ![](assets/prime-dislike.png)   。
* コメント![](assets/prime-dislike.png)好き![](assets/prime-like.png)嫌い。
...
1. 共有するメディアを開きます。
1. 共有ボタンをタップして、「ラーニングマネージャー」を選択します。
1. メディアを共有するボードを選択します。
1. **投稿**&#x200B;をタップします。
...
1. **完了証明**&#x200B;をタップして、PDFや画像などのファイルをアップロードします。
...
* ファイルを送信できるのは、ステータスが **送信保留中および却下**&#x200B;の場合のみです。
* 状態が **承認待ちおよび承認済み**&#x200B;の場合は、ファイルを送信できません。
...
LinkedInアプリを介してコースを受講している場合、進捗状況はラーニングマネージャー内で追跡されません。 したがって、コースはLinkedInラーニングアプリではなくブラウザで起動することをお勧めします。

>[!NOTE]
>
>コースは、アクティブなインターネット接続がある場合にのみ受講できます。

| ![](assets/start-course.jpg) | ![](assets/enroll-course.jpg) | ![](assets/sign-in.jpg) |
|---|---|---|

*LinkedInラーニングにコースを登録する*

LinkedIn 学習コースで費やされた学習時間は、LinkedIn コンテンツ／LinkedIn プラットフォームから、Learning Manager 学習プラットフォームに送信されます。

LinkedIn Learning から学習時間が送信されない場合、学習プラットフォームではその時間を記録できません。このような場合、ラーニングマネージャによって表示される学習時間はゼロです。
...
作成者は、学習者がモバイルアプリで使用する教室モジュールとアクティビティモジュールのみを作成できます。

学習者がコースを開始すると、下のスクリーンショットに示すように、学習者は **ファイル送信** ステータスとアップロードファイルへのリンクを画面に表示できます。
...
ファイルをアップロードすると、状態が [ **承認待ち] に変わります。**. 次に、インストラクターは [保留中の提出物のリストを表示し](/help/migrated/instructors/feature-summary/learners.md) インストラクターWebアプリを介して提出を承認または拒否します。

インストラクターが送信内容を承認すると、学習者モバイルアプリのステータスが「**承認済み**」に変わります。

<!--![](assets/approved.jpg)-->

教員が提出物を拒否すると、学習者モバイルアプリのステータスが **却下**&#x200B;に変わります。

<!--![](assets/rejected.jpg)-->

ファイルを再度送信するには、「**ファイルをアップロード**」リンクをクリックします。

## iPadおよびタブレット用のラーニングマネージャー {#captivateforipadandtablets}

iPadまたはAndroidタブレットのラーニングマネージャーアプリで、学習者としてログインすると、次の **ホーム** 画面が表示されます。

<!--![](assets/screenshot-2015-08-07-12-24-40-e1439211134842.png)-->

学習とカタログ機能に移動するには、 **メニュー** ドロップダウンをタップして適切なオプションを選択します。
...
ユーザーは、緑色のバーの **オンラインコンテンツの表示** ボタンをクリックして、オンラインモードに戻すことができます。 デバイス内のコンテンツは、ユーザーがオンラインモードに切り替えると同期されます。

## モバイル没入型での複数アイテムカートのサポート

学習者は、複数のコース、学習パス、または認定資格をカートに追加して、それらを一緒に購入できるようになりました。 [ ALM で複数品目カートを表示](/help/migrated/learners/feature-summary/multi-item-cart.md)
