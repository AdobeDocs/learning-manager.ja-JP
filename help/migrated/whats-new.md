...
* 以前のリリースの Adobe Learning Manager では、コース、資格認定または学習プランが削除された場合でも、その通知が表示されていれば通知にアクセスしてコース、資格認定または学習プランにアクセスすることができました。 このリリースでは、削除された投稿に確実にアクセスできなくなります。 /posts/でIDを指定した場合{id} APIが実行され、POSTのIDが使用できなくなると、APIは「指定されたリソースのPOSTが見つかりません」というメッセージを表示します。
