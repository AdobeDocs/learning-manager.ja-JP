...
## 取得したコースや学習パスの変更を制限 {#restrictchangesonacquiredcourseslp}

アカウント共有を使用する組織では、学習コンテンツは多くの場合、プライマリアカウントで作成および管理され、共有カタログを通じてピアアカウントと共有されます。 このモデルは、複数のアカウント間でコンテンツの一貫性を維持するのに役立ちますが、アカウントを受け取る際に意図しない変更が生じる可能性もあります。

これにより、作成者がソースアカウントと共有コンテンツの関係を損なう可能性のある変更を行わないようにすることで、共有の学習目標を保護できます。 これにより、コンテンツ所有者は一元化されたガバナンスを維持しながら、ソースアカウントからの更新を受信側アカウントに引き続き反映させることができます。

この機能は、組織の次の点で役立ちます。

* ソースアカウントから共有されるコンテンツの整合性を保持します。
* ソースアカウントとの同期を解除する可能性のある誤った変更を防ぎます。
* 複数のアカウント間でのコンテンツ管理のオーバーヘッドを軽減します。
* ソースアカウントで行われた更新が、引き続き受信アカウントに反映されていることを確認します。
* 作成者が共有コンテンツを変更しようとしたときに明確な警告を表示します。

カタログをソースアカウントから受信側アカウントに共有すると、コース、学習パス、資格認定などの学習目標が受信側アカウントで使用できるようになります。

受信側のアカウントで「取得したコースや学習パスの変更を制限」が有効になっている場合、Adobe Learning Managerでは、共有カタログから継承されたコンテンツの編集が制限されます。 作成者は、共有コンテンツを引き続き使用できますが、ソースアカウントとの関係を断つ可能性のある変更を行うことはできません。

### 共有カタログコンテンツの編集制限を有効にする

**前提条件**

アカウントは、既に共有カタログからコンテンツを受信している必要があります。

1. ALMに管理者としてログインします。
2. **カタログ**&#x200B;を選択します。
3. 共有カタログを選択します。
4. 共有カタログの&#x200B;**「概要」**&#x200B;セクションで、切り替え&#x200B;**「取得したコース/学習パスの変更を制限」**&#x200B;を選択します。 有効にすると、取得したカタログのコース/学習パスの一部に対する変更が制限され、メインアカウントからこのアカウントへの更新が許可されます。
...
共有カタログを通じて取得したコースを作成者が開くと、システムにより次の内容を示す情報メッセージが表示されます。

* コースは共有カタログから始まります。
* 編集制限が有効になります。
* 特定のコンテンツの変更は制限されています。

### 制限されたコンテンツの編集を試みる

作成者が保護されたコンテンツを変更しようとした場合：

* 作成者には、コースがソースアカウントから取得されたことを示す警告が表示される場合があります。
* この警告は、コンテンツを変更するとソースアカウントとの関係が壊れる可能性があることを説明しています。
* 設定に応じて、システムは影響を受けるコンテンツへの変更を制限または禁止します。

これにより、作成者は変更前にアクションの影響を理解できます。

**どのような変更が制限されていますか？**

編集制限を有効にすると、継承されたコースコンテンツのコンポーネントはソースアカウントで管理されるため、そのコンテンツに対する変更が制限されます。 これらの制限は、アカウント間の同期を維持するように設計されています。

通常ソースアカウントによって管理されるコンテンツの例には、次のものがあります。
...
この目的は、受信側アカウントのコピーがソースバージョンから移行する原因となる変更を防ぐことです。

**許可されている変更は何ですか？**

一部の設定は、引き続き受信側アカウント内で個別に管理され、編集可能な状態が維持されます。 次のとおりです。

* 電子メールテンプレートのカスタマイズ
* インスタンス管理
* ソースアカウントから取得されないその他のアカウントレベル設定。

### シナリオの例

#### 制限前

企業の学習チームは、ソースアカウントでコンプライアンスコースを維持し、複数の地域アカウントとコースを共有します。

地域アカウントの作成者は、取得したコースを直接編集します。 その結果、元のコースとの接続が中断され、ソースアカウントからの今後の更新が正しく流れなくなる可能性があります。

#### 制限後

受信側のアカウントでは、**取得したコースや学習パスの変更を制限**&#x200B;できます。

地域の作成者が共有コースにアクセスすると、コンテンツを表示して割り当てることができますが、保護されたフィールドは変更できません。 ソースアカウントによって行われた更新は、引き続き中断することなく受信側アカウントに配信されます。
