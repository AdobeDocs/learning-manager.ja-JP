...
>[!NOTE]
>
>削除したユーザーがシステムに追加され直すと、以前の役割（管理者、マネージャー、作成者、インストラクターなど） は保持されません。学習者の役割とともに追加されます。
...
<table>
 <tbody>
  ...
  <tr>
   <td><strong>Learning Manager UI- Non-Enterprise を使用した消去</strong></td>
   <td> </td>
  </tr>
  <tr>
  ...
  <tr>
   <td><strong>その他のユーザーの消去 - Enterprise（社内ユーザーまたは外部の Learning Manager ユーザーでない場合）</strong></td>
   <td> </td>
  </tr>
  <tr>
...
一括削除フローでは、1つのバッチにつき最大10,000人のユーザーを処理できます。 この機能は、大量の削除を行うための、信頼性に重点を置いたバックグラウンドのサービスとして位置づけられています。

管理者は、消去されたユーザーを削除できる期間を指定できます。 詳細については、[管理者設定](/help/migrated/administrators/feature-summary/settings.md)を参照してください。

#### 機能：

* アカウントレベルで削除されたユーザーに設定可能な自動消去を提供
* ユーザーがパージ基準を満たしたら、24時間以内に確実にパージする
* システムのパフォーマンスを低下させることなく**1日あたり最大10,000人のユーザーの一括削除をサポート
* これらのオペレーションの実行中に、システム全体のレスポンスとデータベースの状態を維持
* GDPRの義務に対応するためにデータ保存管理の自動化を適用

#### 主な機能：

* 一括削除フローは、スケジュールされたcronジョブとしてのみ実行されます（リクエストごとのオンデマンドではありません）。

### 自動消去オプションの有効化

1. 管理者としてAdobe Learning Managerにログインします。
2. **Configure**&#x200B;セクション> **Settings** > **Basics**&#x200B;セクション> **General**&#x200B;に移動します。
3. ページを下にスクロールして、**削除されたユーザーを自動消去**&#x200B;します。
   ![](assets/auto-purge1.png)
   *自動消去オプション*
   >[!NOTE]
   >
   >**削除されたユーザーの自動消去**&#x200B;が有効になっていない場合、セクションには、オプションのセクションの下部に「**未構成**」というメッセージが表示されます。
4. 「**編集**」を選択します。
5. 「**有効にする」チェックボックス**&#x200B;をオンにします。
6. パージを有効にする期間を入力します。
   >[!NOTE]
   >
   >最小値は1年である必要があります。 また、1つずつ増加することもできます。 ただし、1.5年、2.5年などの値は入力できません。 期間としてカスタム値が必要な場合は、カスタマーサポートにお問い合わせください。
7. 「**保存**」を選択します。 ALMに詳細な確認メッセージが表示されます。
   ![](assets/auto-purge2.png)
   *ライセンス認証を行って期間を入力しています*
8. **はい**&#x200B;を選択して設定を確認し、保存します。
...
+++Adobe Learning Managerで一括消去を実行できますか？
...
掲示板で 一度消去すると、すべてのユーザーデータは完全に削除され、復元することはできません。
