...
1. スキルにドメインを割り当てます。 スキルは作成時に、Learning Manager がサポートする最も関連性の高いスキルドメインにマッピングすることができます。 詳細については、[***ドメインへのスキルのマッピング***](/help/migrated/administrators/feature-summary/curation-skills.md)&#x200B;を参照してください。
...
管理者は、**スキルを追加**&#x200B;ダイアログで一括アップロードオプションを使用して、Adobe Learning Manager内で複数のスキルを直接追加できます。

スキルを一括で追加するには、次の手順を実行します。

1. 左ペインから「**スキル**」を選択します。

1. **追加** > **スキルの追加**&#x200B;を選択します。<br> **スキルの追加**&#x200B;のポップアップウィンドウが表示されます。

1. 「**スキルを一括で追加（CSVをアップロード）**」を選択します。

   ![「CSVをアップロード」オプションを使用したスキルの一括追加ダイアログ](assets/add-skills-bulk-upload-csv.png)
   *スキルを一括追加するオプションを表示するスキルの追加ポップアップウィンドウ。*

1. スキルの詳細を記載したCSVファイルをアップロードします。
...
**[!UICONTROL スキル]**&#x200B;ページで、変更するスキルをクリックします。 **[!UICONTROL スキルを編集]**&#x200B;ダイアログで、必要な変更を行います。例えば、以下のような変更を行うことができます。
...
1. CSV をアップロードします。 CSV には次の列が必要です。

   * スキル名
   * スキルレベル
   * インストラクターの電子メールまたはインストラクターの UUID

   UUID が有効なアカウントの場合、「インストラクターの電子メール」列を「インストラクターの UUID」に置き換えます。
...
+++バッジとスキルを有効にして、コース完了に関連付けるにはどうすればよいですか？
...
+++管理者は、バッジに「進行中」と表示されていても、バッジを完了としてマークできますか？
...
* [スキル/Adobe学習マネージャー](https://elearning.adobe.com/2018/11/skills-captivate-prime/)