...
登録、出席、完了に同じCSVファイル形式を使用できます。 「電子メール」列に学習者の電子メールIDを入力し、アクションに基づいた名前を付けてファイルを保存するだけです。例えば、bulk_enrollment.csv、bulk_attendance.csv、bulk_completion.csvなどです。 CSV形式のみがサポートされています。 UTF-8形式はサポートされていません。 [サンプルcsv](/help/migrated/administrators/assets/Sample-Bulk-Action-CSV.csv)をダウンロードします。
