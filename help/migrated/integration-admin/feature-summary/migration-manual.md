...
従業員AがLearning Managerという名前の会社に契約社員として入社するシナリオを考えてみます。 契約期間中、Learning Manager会社は会社の電子メールIDを ```A@example.com```代わりに、会社は従業員の個人のメールアカウントのみを考慮して、次のように記述します。 ```A@gmail.com```. 契約期間が6か月を完了した後、同じ従業員Aがフルタイムの従業員としてLearning Managerに参加する場合、Learning Managerでは電子メールIDを会社の電子メールIDに変更する必要があります。 ```A@example.com```.
