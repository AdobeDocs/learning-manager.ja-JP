...
* 特定のプラットフォームに表示する必要がある学習目標のみを参照します。
...
* 明示的にフィルターされない限り、すべてのアカウントコンテンツが意図せずに表示される可能性があります
* ツールプロバイダーの統合は操作が困難
...
5. プラットフォーム固有のカタログ公開のサポート
   * 管理者は、どのカタログをどのLTIプラットフォームに表示するかを制御できます。
...
*これらのパラメーターは、どのコンテンツを表示し、選択内容をどのようにALMに返すかを制御します。*

## ディープリンクの作成

>[!IMPORTANT]
>
>以下のセクションおよび著者のセクションで説明する手順は、ALM側ではなく、コースを使用するプラットフォームで実行する必要があります。

### 前提条件

1. 統合管理者としてログインする必要があります。
2. LTI統合を設定する際に、[**ディープリンクをサポート**]チェックボックスをオンにします。 [LTI統合](/help/migrated/integration-admin/feature-summary/learning-tools-interoperability.md)を設定する方法を説明します。
3. ユーザーまたは作成者が選択内容にアクセスするためのURLをフィールドに入力します。
4. 「**変更を保存**」を選択します。

   同じ起動URLが再利用され、設定と使用が簡素化されます。

   動作は、LTIメッセージの種類によって決まります。 メッセージの種類が`content_consumption`の場合、ユーザーはコースプレーヤーにリダイレクトされます。 メッセージの種類が`content_selection`の場合、ユーザーはディープリンクフローを使用してルーティングされます。作成者は、コース固有の識別子を手動でコピーすることなく、目的のコンテンツを直接選択できます。
