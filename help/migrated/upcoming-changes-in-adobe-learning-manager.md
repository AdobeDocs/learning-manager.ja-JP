---
title: Adobe Learning Managerの新機能 2026 年 4 月リリース
description: Adobe Learning Manager 2026 年 4 月リリースの新機能、改善点および重要な更新について説明します。
...
# Adobe Learning Managerの更新

>[!IMPORTANT]
>
>このリリースの機能はベータ版です。 機能と動作は、一般リリース前に変更される場合があります。 通常のAdobe サポートチャネルを通じてフィードバックをお寄せください。


このドキュメントでは、Adobe Learning Managerの 2026 年 4 月リリースの新機能、改善点および更新について説明します。 組織の変更を計画し、学習者、管理者および作成者が使用できる機能を理解するために使用します。

**学習者の場合：** Fluidic プレーヤーに次のモジュール名が表示され、「終了」ボタンがクリアされます。 プレーヤーの言語を LTI を介して設定し、プラットフォーム間で一貫したエクスペリエンスを実現できます。 Captivate コンテンツには、統一された目次、スライドレベルの完了クォート、信頼性の高いメモの書き出しが含まれています。 作業計画書、チェックリストの質問、ビデオテキストトラック（VTT）で多言語サポートを利用できます。 AI アシスタントは、学習者が学習体験の中で回答を得るのを支援します。

**管理者および作成者の場合：** ズームコネクタは、複数の同時 VILT セッションをサポートします。 ピアアカウントの共有コースに、「外部作成者」ではなく実際の作成者が表示される。 管理者は、モジュールを開始できるタイミングを制限できます。 学習オブジェクトの有効期限は、学習者 API で公開されます。 チェックリストモジュールは、重み付けスコアリング、多言語質問テキストおよびオプションのレビュー担当者コメントをサポートしています。 カスタム証明書は、動的フィールドと AI が生成した背景を含むドラッグアンドドロップエディターを提供します。 ログインしていない Experience Builder では、ログインを必要とせずにパブリック学習ページを作成できます。

**インストラクターの場合：** インスタンスの登録とセッションの出席に関する QR コードを生成します。 チェックリストの評価時にコメントまたはフィードバックを追加できます。

**レポートと分析：** SCORM コンテンツは、L2 レポートで複数のクイズの試行をレポートできるようになりました。 学習時間の計算が、学習者のトランスクリプトで改善されました。 管理者向けの学習トランスクリプトレポートが更新されました。 高度な検索の機能強化が利用できます。
...
* 学習者は、チェックリスト評価が発生するたびに通知（プラットフォーム内および電子メール）を受け取ります。 次のような通知があります。

   * コメントと

   * レビュー担当者の名前（表示可能に設定されている場合）。

これにより、フィードバックが保存されるだけでなく、学習者に積極的に表示されます。

### 主なメリット

* **コーチのような豊かなフィードバック**：数値スコアにはコンテキストに基づくコメントが追加されるため、チェックリストはコンプライアンスだけでなく、コーチングに対してより効果的なツールとなります。

* **トレーサビリティと可監査性**：組織は、誰が、いつ、何を発言したかを評価した永続的な記録を得ることができます。これは、規制対象の環境や高リスクの役割において重要です。

* **学習者のエンゲージメントの向上**：学習者は、特定の評価に関連する明確なガイダンスを受けることで、期待とそれに続く手順の理解を深めることができます。

### ユースケース

* 規制環境を持つ組織は、現場で観察されているスタッフの臨床判断や手順のフィードバックを文書化するためにコメントを使用できます。

* 航空機関や地上対応機関は、運用実績、安全慣行、顧客対応行動に関する詳細なメモを添付できるので、チェックリストは構造化された報告書作成ツールに変わります。

* メンタリングや SME 評価では、教師は、「エスカレーションをうまく処理したが、時間管理を改善する必要がある」、「優れたトラブルシューティングフロー、ドキュメント手順の欠席」など、スコアだけでは収まらない微妙な考察を取り込むことができます。

## コンテンツレベルでの複数回の試行とクイズレポート

### 概要

>[!IMPORTANT]
>
>この機能は、アカウントで有効にした後でのみ使用できます。 ALM サポートに連絡してください。


現在、ALM は Multiple Quiz Attempt （MQA；複数テスト試行）機能を使用して、LMS レベルでの複数の試行をサポートしています。

* 作成者は、コースレベル（コース内のすべてのクイズベアリングモジュールに適用）またはモジュールレベル（クイズモジュールごと）で試行回数を設定できます。

* 試行できる操作：

   * 特定の数（例：3 回の試行）、

   * LMS レベルで制御される、試行回数制限なし。

* 学習者が Fluidic Player でモジュールを使用してから、プレーヤーを閉じるかモジュールを完了すると、そのセッションは単一の LMS 試行として扱われます。

* LMS の試行は、L2 クイズレポートで新しい行として取り込まれます。

ただし、コンテンツ・ファイル自体（SCORM クイズの説明など）が独自の複数回試行ロジックを実装している場合、ALM の L2 クイズ・レポートでは、現在、それらの内部試行が正しく区別または追跡されていません。

この機能強化により、クイズにコンテンツレベルの複数回の試行追跡が導入され、Adobe Learning Managerで L2 クイズレポートのコンテンツ内の各試行を正確に取得できるようになりました。 これは、コンテンツ・オーサリング・ツール（SCORM の説明など）がクイズの試行を個別に管理する場合に使用します。 この機能を使用すると、LMS レベルの Multiple Quiz Attempt （MQA）設定に依存することなく、試行回数が ALM レポートに正しく反映されます。
...
1. コンテンツライブラリに移動し、**追加/コンテンツ**&#x200B;を選択します。
...
アクセシブルなカタログに登録されていない学習者でも、登録されている学習目標を検索結果で確認できるようになりました。これにより、関連するコンテンツを見逃すことはありません。 さらに、作業計画書のランキングが詳細検索とコンテンツ内検索の両方で改善され、最も関連性の高いリソースがより迅速に表示されるようになりました。
