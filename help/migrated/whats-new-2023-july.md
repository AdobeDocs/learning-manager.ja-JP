...
>
>レポートの自動化設定により 1 コースにつき 1 行しか予測されていない場合は、複数登録機能を有効にする前に、レポートの自動化に必要な調整を行う必要があります。
...
この変更により、ExaVault コネクタも置き換えられ、新規ユーザは ExaVault コネクタを使用できなくなります。 ExaVault の代わりに、任意のオープンソース FTP クライアントを使用できます。 詳しくは、[Adobe FTP Manager からの移行](transition-from-ftp-manager.md)を参照してください。
...
ステミングは、単語を語根の形式に短縮するプロセスです。 これにより、検索時に用いる単語の活用形を一致させることができます。 例えば、walking と walked は同じ語根である walk にステミングできます。 語幹検索では、どちらかの単語の出現箇所は他方の単語と一致します。

このリリースでは、英語ロケール用のステミングが追加されました。これには、en_US、en_AU、en_GB のバージョンが含まれます。
...
* Harvard Manage Mentor、コンテンツマーケットプレイス、getAbstract または LinkedIn のコース、学習プラン、または資格認定。
* 前提条件が有効になっている学習プランおよび資格認定。
* 廃止済みコース、学習プランまたは資格認定。
* 有効期限が切れているコース、学習プラン、または資格認定。
* 外部の資格認定。
* e コマースが有効になっているコース、学習プランまたは資格認定。
...
* 移行中はマッピング不可能。
* LinkedIn、コンテンツマーケットプレイスおよびその他の外部コース、学習プランまたは資格認定の自動タグ付け。
* 本番環境移行後にスキルベースまたはクラシックに戻すこと。
* 学習者アプリの製品と役割の検索メニュー。
* 管理者アプリ内でのコース、学習プランまたは資格認定およびユーザーの一括マッピング。
